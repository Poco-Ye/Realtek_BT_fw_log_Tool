...
#define MSG_MBOX_IN_COMMAND_PARAM 3406 //"[MB] wl2bt => %08x %08x"
#define MSG_MBOX_OUT_COMMAND_PARAM 3407 //"[MB] bt2wl <= %08x %08x"
#define MSG_RT8723_RF_THERMAL_TIMER_NEW 3408 //"NewThermalTimerCallback..thermal:(now %d diff %d default %d), track:(txpwr %d cfo %d), mode = %d"
#define MSG_HCI_VENDOR_SET_THERMAL_VALUE_AND_POUTER 3409 //"HciVendor: Pouter Mode En %d, current_thermal %d"
...
#define DAPE_TEST_LOG563 4663 //" Send Disconnect Complete Event to Host at Host Suspend. Disconn Reason 0x%02x."
#define LOG_PEND_PKT_ON_HOST_SUSPENSION 4664 //" Pend packet (type: %d, data: %02x:%02x:%02x:%02x:%02x:%02x) on host suspension"
#define LOG_SEND_PKT_ON_HOST_RESUMPTION 4665 //" Send packet (type: %d, data: %02x:%02x:%02x:%02x:%02x:%02x) on host resumption"
...
#define PTA_DISPLAY_BT_INFO_MESSAGE 12041 //"<PTA> BT MESSAGE Connection=  %d SCO= %d INQ= %d ACL=%d SCO busy= %d TXRetry=%d  HID=%d A2DP=%d FTP=%d ACLTXCnt=%d ACLRXCnt=%d RSSI=0x%x"
...
#define MAILBOX_PATCH 12056 //"<MAILBOX> HCI reset replace patch"
#define PTA_DISPLAY_BT_INFO_MESSAGE4 12057 //"<PTA> Conn=%d SCO=%d INQ=%d ACL=%d SCO busy=%d TXRetry=%d HID=%d A2DP=%d FTP=%d out_data2 %x rssi_sum %d pkt_cnt=%d Page_only=%d "
...
#define PTA_DISPLAY_BT_INFO_MESSAGE5 12063 //"<PTA> Conn=%d SCO=%d INQ=%d ACL=%d SCO busy=%d TXRetry=%d HID=%d A2DP=%d FTP=%d out_data2 %x rssi_sum %d pkt_cnt=%d Page_only=%d bitpool = %d"
#define PROFILE_INFO_CHECK 12064 //"busy profile info , SCO_bit[0]=%d HID_bit[1]=%d A2DP_bit[2]=%d FTP_bit[3]=%d number of HID = %d / %d  OPP_RX = %d"
#define BT_REPORT_REDUCED_LOG 12065 //"<coex info> INQ/PAGE = %d ACL=%d SCO busy=%d HID=%d A2DP=%d FTP=%d rssi_value=%d a2dp_bitpool=%d"
#define BT_REPORT_REDUCED_LOG1 12066 //"<coex info> INQ/PAGE = %d ACL=%d SCO busy=%d HID=%d A2DP=%d FTP=%d rssi_value=%d"
#define DRIVER_REPORT_BITPOOL 12067 //"driver provide a2dp bitpool info = %x"
#define PTA_COEX_INFO 12068 //"<Bt Coex Info> LowByte = 0x%x , HighByte = 0x%x"
