...
#define DAPE_TEST_LOG441 4141 //"Adv Match MSFT's filter. Send Adv event. Mon_type %d adv_entry %d bm0x%02x send_1st_adv %d sample_period %d"
...
#define DAPE_TEST_LOG553 4653 //" WHCK running...Item:%d (0:SCO 1:P2P 2:2.1LOGO 3:LE LOGO 4:Dual mode 5:SUT 6:S_PNP 7:4LE(R))"
...
#define DAPE_TEST_LOG588 4688 //" Drop ACL pkt(0x%02x %02x %02x %02x %02x) because Host is suspend(clk 0x%08x)"
#define DAPE_TEST_LOG589 4689 //" Detect Host Suspend. (clk 0x%08x)"
#define DAPE_TEST_LOG590 4690 //" Detect Host Leave Suspend! (clk 0x%08x)"
#define DAPE_TEST_LOG591 4691 //" Send Disconnect Complete Event to Host at Host Suspend. Disconn Reason 0x%02x."
#define DAPE_TEST_LOG592 4692 //" Warning! Bluewiz State Machine stuck @ %d (2: TX_SIE 8:RX_SIE)"

#define LOG_PEND_PKT_ON_HOST_SUSPENSION 4900 //" Pend packet (type: %d, data: %02x:%02x:%02x:%02x:%02x:%02x) on host suspension"
#define LOG_SEND_PKT_ON_HOST_RESUMPTION 4901 //" Send packet (type: %d, data: %02x:%02x:%02x:%02x:%02x:%02x) on host resumption"
...
#define BRUCE_TEST_LOG5615 5615 //" (PSD)recovery III caseB %d"
#define BRUCE_TEST_LOG5616 5616 //" (PSD)afh_map: %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X, %02X, th1(%d)"
#define BRUCE_TEST_LOG5617 5617 //" (PSD)Warning!!No completed any PSD scan, No update afh_map_modem_psd %d %d"
...
#define DISALLOW_CREATE_CONN_WHEN_HW_DISCONNECTING 5630 //"Disallow Host create connection because FW found same BD_ADDR disconnecting(%d)"

#define MSG_BT_WDG_TIMEOUT_DBG 5800 //"Wdg_Timeout!!!(pre_stats) isr %d, isr_bm %02x,  cpu_idle %d, sig_id %d, isr_cause %08x, isr_cnt %d, last_epc 0x%08x"
...
#define FONGPIN_USB_L1_WAKEUP_WHEN_TIMER           8034    //"(usb) usb L1 remote wakeup when timer"
...
#define PACKAGE_TO_HCI                              8041    //"(hci) hci selection = %d,(0:S-UR, 1:U-U, 2: ME-U, 3:E-U, 6:E-UR, 7: ME-UR)"
#define PACKAGE_ERROR_SELECTION                              8042    //"(hci) hci selection error"
#define PAGE0_REG_HW_INFO                             8043    //"(system on) hw id = 0x%X (9: 8821C, A:8822B, B:8703B, F:8723D ), chip version = 0x%X"
...
#define PTA_COEX_INFO 12053 //"<Bt Coex Info> LowByte = 0x%x , HighByte = 0x%x"
#define COEX_RELEASE_WIFI_CONTROL_BT_TX_POWER 12055 //"!! IMPORTANT!! wifi off , recover the thermal tracking power step before wifi assign"
#define COEX_WIFI_CONTROL_BT_TX_POWER 12056 //"!! IMPORTANT!! wifi set bt tx power diff step :%x(INT8) , pausing thermal tracking!! "
...
#define MP_PKT_TRX_CMD_PARSER 12807 //"pattern:%x, tx_interval:%x, tx_power_index:%x, is_whiten:%x, whiten_coeff:%x, rf_chnl:%x, pkt_type=%x, multi_rx_enable:%x, is_pseudo_outer:%x, is_pkt_tx:%x, header:%x, tx_pkt_cnt:%x, length:%x, target_bt_addr_out:%x %x %x"
...
#define MP_SET_TX_POWER 12815 //"tx_power_index: 0x%x, gain: 0x%x"
...
#define LOG_TV_AUTO_PAIR_AD_MATCH 13024 //"[TV] ad match: ad_len: %d, ad_data: %02x:%02x"
#define LOG_TV_AUTO_PAIR_AD_RCU_ADDRESS_CHECK 13025 //"[TV] ad match: rcu addr: %02x:%02x:%02x:%02x:%02x:%02x"
#define LOG_TV_AUTO_PAIR_AD_RCU_ADDRESS_MASK 13026 //"[TV] ad match: rcu addr mask: %02x:%02x:%02x:%02x:%02x:%02x"
#define LOG_TV_AUTO_PAIR_AD_RCU_ADDRESS 13027 //"[TV] ad match: rcu addr to check: %02x:%02x:%02x:%02x:%02x:%02x"
#define LOG_TV_AUTO_PAIR_AD_RCU_ADDRESS_CHECK_RESULT 13028 //"[TV] ad match: rcu addr check result :%d"
#define LOG_TV_AUTO_PAIR_AD_CHECK_TYPE 13029 //"[TV] ad match: ad type to check: %02x"
#define LOG_TV_AUTO_PAIR_CHECK 13030 //"[TV] ad match: auto-pair enable: %d, check_data_len: %d"
#define LOG_TV_HOST_SUS_DROP_EVENT 13031 //" Drop Event(0x%x) because Host is suspend (clk 0x%08x)"
...
#define IQK_LOK_RESULT 18030 //"LOK = 0x%x , IQK_x = 0x%x , IQK_y = 0x%x , current_path =%x , success = %x , K_path =%x , ANT_EFUSE = %x"
#define SCOREBOARD_DATA_WRITE 18031 //"SCOREBOARD data write : %x"
#define SCOREBOARD_DATA_READ 18032 //"SCOREBOARD data read : %x"
#define IQK_LOK_RESULT_SINGLE 18033 //"BT signle iqk/lok result , LOK = %x , IQK_x = %x , IQK_y = %x , check_cnt = %x , fail=%x, valid=%x"
#define FIND_OUT_WIFI 18034 //" DETECT WIFI alive first time !! wake up from LPS mode and extend lps_mode_max_time"
#define ANT_DETECTION_DONE 18035 //" ANT_DETECTION finish!"
#define RECEIVE_SCOREBOARD 18036 //" Receive scoreboard data = 0x%x"

#define PHY_INIT_LOK_IQK_RESULT 19000 //"[LOK/LOK result] , check cnt: %d ,LOK_result : 0x%04x ,IQK_result_x : 0x%04x ,IQK_result_y : 0x%04x"
#define FC19_CMD_PARSE 19001 //"[Profile Info] SCO_bit[0]=%d HID_bit[1]=%d A2DP_bit[2]=%d FTP_bit[3]=%d number of HID = %d"
#define DRIVER_REPORT_BITPOOL 19002 //"driver provide a2dp bitpool info = %x"
#define FT_VALUE_VALID 19003 //"FT value is valid !!"
#define FT_VALUE_INVALID 19004 //"FT value is invalid :("

#define FIQK_FLOK_LOG3 19005 //"const = %d , original_real = %d , original_imag = %d , after_real = %d, after_imag = %d"
#define FIQK_FLOK_LOG4 19006 //"value1_real = %d , value1_imag = %d , value2_real = %d , value2_imag = %d , after_real = %d, after_imag = %d"
#define FLOK_BLOCK_A 19007 //"1st Iteration FLOK_BLOCK_A ****  p0_r = %d , p0_i = %d , p1_r = %d , p1_i = %d"
#define FLOK_BLOCK_B 19008 //"1st Iteration FLOK_BLOCK_B ****  ar_t = %d , ai_t = %d , br_t_r = %d , bi_t = %d"
#define FLOK_BLOCK_C 19009 //"1st Iteration FLOK_BLOCK_C ****  ar = %d , ai = %d , br = %d , bi = %d"
#define FLOK_BLOCK_D 19010 //"1st Iteration FLOK_BLOCK_D ****  x_tmp = %d , y_tmp = %d , divisor = %d"
#define FLOK_BLOCK_E 19011 //"1st Iteration FLOK_BLOCK_E ****  iDAC_i = %d , iDAC_q = %d "
#define FLOK_BLOCK_A_2 19012 //"2nd Iteration FLOK_BLOCK_A ****  p0_r = %d , p0_i = %d , p1_r = %d , p1_i = %d"
#define FLOK_BLOCK_B_2 19013 //"2nd Iteration FLOK_BLOCK_B ****  ar_t = %d , ai_t = %d , br_t_r = %d , bi_t = %d"
#define FLOK_BLOCK_C_2 19014 //"2nd Iteration FLOK_BLOCK_C ****  ar = %d , ai = %d , br = %d , bi = %d"
#define FLOK_BLOCK_D_2 19015 //"2nd Iteration FLOK_BLOCK_D ****  x_tmp = %d , y_tmp = %d , divisor = %d"
#define FLOK_BLOCK_E_2 19016 //"**** FLOK final result  ****  iDAC_i = %d , iDAC_q = %d , ant at = %d"
#define FIQK_BLOCK_A 19017 //"1st Iteration FIQK_BLOCK_A ****  after Q p0_r = %d , p0_i = %d , p1_r = %d , p1_i = %d , p2_r = %d , p2_i = %d"
#define FIQK_BLOCK_B 19018 //"1st Iteration FIQK_BLOCK_B ****  ar_t = %d , ai_t = %d , br_t = %d , bi_t = %d"
#define FIQK_BLOCK_C 19019 //"1st Iteration FIQK_BLOCK_C ****  ar = %d , ai = %d , br = %d , bi = %d"
#define FIQK_BLOCK_D 19020 //"1st Iteration FIQK_BLOCK_D ****  x_tmp = %d , y_tmp = %d , divisor = %d"
#define FIQK_BLOCK_E 19021 //"1st Iteration FLOK_BLOCK_E ****  X_fixed = %d , Y_fixed = %d "
#define FIQK_BLOCK_A_2 19022 //"2nd Iteration FIQK_BLOCK_A ****  after Q p0_r = %d , p0_i = %d , p1_r = %d , p1_i = %d , p2_r = %d , p2_i = %d"
#define FIQK_BLOCK_B_2 19023 //"2nd Iteration FIQK_BLOCK_B ****  ar_t = %d , ai_t = %d , br_t = %d , bi_t = %d"
#define FIQK_BLOCK_C_2 19024 //"2nd Iteration FIQK_BLOCK_C ****  ar = %d , ai = %d , br = %d , bi = %d"
#define FIQK_BLOCK_D_2 19025 //"2nd Iteration FIQK_BLOCK_D ****  x_tmp = %d , y_tmp = %d , divisor = %d"
#define FIQK_BLOCK_E_2 19026 //" **** FIQK final result  ****  X_fixed = %d , Y_fixed = %d , ant at = %d"
#define FIQK_FLOK_EXCEPTION 19027 //" !!WARNING , FIQK,FLOK exception happened!!!"
#define FIQK_FLOK_LOG1 19028 //"a_31:0 = %x , a_39:32 = %x , a_deg = %x , b_31:0 = %x , b_39:32 = %x , b_deg = %x"
#define FIQK_FLOK_LOG2 19029 //"psd0_real = %x , psd0_imag = %x , psd1_real = %x , psd1_imag = %x , cal_tmp0_real = %x , cal_tmp0_imag = %x , cal_tmp1_real = %x , cal_tmp1_imag = %x , result_real = %x , result_imag = %x"

#define L2CAP_ACL_SIGNAL_CONN_REQ 19500 //"[L2CAP SIG][Tx: %d][Conn req] conn_handle: %d, profile_idx: 0x%X, local_cid: 0x%X, remote_cid: 0x%X"
#define L2CAP_ACL_SIGNAL_CONN_RESP 19501 //"[L2CAP SIG][Tx: %d][Conn resp] conn_handle: %d, profile_idx: 0x%X, local_cid: 0x%X, remote_cid: 0x%X"
#define L2CAP_ACL_SIGNAL_DISCONN_REQ 19502 //"[L2CAP SIG][Tx: %d][DisConn req] conn_handle: %d, idx: 0x%X, local_cid: 0x%X, remote_cid: 0x%X"
#define L2CAP_DENIED_PROFILE_REPORT 19503 //"[L2CAP] Denied profile report from host"

#define COEX_STATUS_SUMMARY 19600 //"[Coex summary] wl_score: %x, bt_score: %x, Tx_pwr: %x, LNA: %x(%x), AFH: %02x %02x %02x %02x %02x %02x %02x %02x %02x %02x"

// ============== RF related section ========
#define RF_THERMAL_TRACKING_TMETER_DBG     25002 //"thermal:(now: 0x%x, default: 0x%x, delta: 0x%x, diff: %d, diff_step: %d), track_en: %d, ll_cur_mode: %d"
#define RF_THERMAL_TRACKING_POWER_DBG      25003 //"default_power_index(max: 0x%x, 1M: 0x%x, 2M: 0x%x, 3M: 0x%x, LE: 0x%x), cur_power_index: 0x%x, step_index(default: 0x%x, max: 0x%x)"
