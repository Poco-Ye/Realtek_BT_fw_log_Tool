...
#define LC_ISR_2104  1472  //"clock_of_piconet %x, piconet_id %x , reserved_buffers %x , free_pools %x"
...
#define MSG_IQK_FLOW 2951 //"[BT IQK flow] %d %d %d %d"
#define MSG_RF_REG_READ 2952 //"[BT RF Reg Read] 0x%02x = 0x%04x"
...
#define MSG_WIFI_MP_DL_CMD_DBG 3442 //"mbox_dl_cmd : addr=%08x, len = %d, mode = %d"
#define MSG_WIFI_MP_DL_DATA_DBG 3443 //"mbox_dl_data: offset=%04x: %08x %08x %08x %08x - %08x %08x %08x %08x - %08x %08x %08x %08x - %08x %08x %08x %08x"
#define MSG_BT_WDG_TIMEOUT_DBG 3444 //"Wdg_Timeout!!!(pre_stats) isr %d, isr_bm %02x,  cpu_idle %d, sig_id %d, isr_cause %08x, isr_cnt %d last_epc %08x"
#define MSG_GIVE_UP_LOK 3445 //"Warning !! Give Up LOK because BT Air Protocol is Beginning!!"

#define MSG_DISPLAY_FREE_TIMER_LIST 3498 //"[*(%d) timer id %02d] %02d %02d %02d %02d :  %02d %02d %02d %02d : %02d %02d %02d %02d : %02d %02d %02d %02d"
...
#define MSG_CP3_COUNTER_LOG             3711 //"[CP3 for MIPS] mips_st_max = %d.%02d (new %08x miss %08x mbc %08x),  mips_lt_adv = %d.%02d  (new %08x miss %08x mbc %08x)"
...
#define DAPE_TEST_LOG553 4653 //" WHCK running...Item:%d (0:SCO 1:P2P(%d) 2:2.1LOGO 3:LE LOGO 4:Dual mode 5:SUT 6:S_PNP 7:4LE(R))"
...
#define PTA_DISPLAY_BT_INFO_MESSAGE4 12058 //"<PTA> Conn=%d SCO=%d INQ=%d ACL=%d SCO busy=%d TXRetry=%d HID=%d A2DP=%d FTP=%d out_data2 %x rssi_sum %d pkt_cnt=%d Page_only=%d TRxMask=%d"
#define PAGE_CHANGE_CHECK 12059 //"<PAGE checek , register should be oxffe mean HIGH>"
#define PAGE_CHANGE_CHECK1 12060 //"<PAGE checek> out_data1=%x"
#define HID_PRIORITY_DEBUG_CHECKK 12061 //"<0x190 input check> register_0x190=%2X"
#define HID_PRIORITY_DEBUG_CHECKK1 12062 //"<0x190 output check> register_0x190=%2X"
#define PTA_DISPLAY_BT_INFO_MESSAGE10 12063 //"pta_meter_var.dwPtaACLRxCnt = %d  ,  pta_meter_var.dwPtaACLTxCnt = %d"
#define PROFILE_INFO_CHECK 12064 //"busy profile info , SCO_bit[0]=%d HID_bit[1]=%d A2DP_bit[2]=%d FTP_bit[3]=%d number of HID = %d"
#define PROFILE_INFO_CHECK1 12065 //"SCO_bit[0]=%d , handle = %x , A2DP_bit[2]=%d  , handle = %x , FTP_bit[3]=%d , handle =%x"
#define IQK_LOK_VALUE_WITH_FT	12066	//"IQK_LOK_value from FT , patch_g_ft_efuse_manage.ft_efuse_records.d8[%d] = %x"
#define PAGE_EXTEND_TIMER_START	12067	//"EXTEND PAGING PERIOD %d s START"
#define PAGE_EXTEND_TIMER_STOP	12068	//"EXTEND PAGING PERIOD  STOP"
#define PTA_COEX_INFO 12069 //"<Bt Coex Info> LowByte = 0x%x , HighByte = 0x%x"
#define DD_MSG_CMD11 12086 //"<3DD> LE scan:%d(int:%d,win:%d),dup:%d. Legacy scan:%d. (ps:%d/%d slots)(is:%d/%d slots)"
#define PATCH_3_DH5_DEBUG0  12100 //"????????       lc_tx_status = %x , tx_pkt_type=%x , native CLOCK=%x"
#define PATCH_3_DH5_DEBUG1  12101 //"????????       lut_index_lower = %x , lut_index_upper=%x "
#define PTA_DISPLAY_BT_INFO_MESSAGE5 12102 //"<PTA> Conn=%d SCO=%d INQ=%d ACL=%d SCO busy=%d TXRetry=%d HID=%d A2DP=%d FTP=%d out_data2 %x rssi_sum %d pkt_cnt=%d Page_only=%d TRxMask=%d bitpool = %d"
#define BT_REPORT_REDUCED_LOG 12103 //"<coex info> INQ/PAGE = %d ACL=%d SCO busy=%d HID=%d A2DP=%d FTP=%d rssi_value=%d a2dp_bitpool=%d"
#define DRIVER_REPORT_BITPOOL 12104 //"driver provide a2dp bitpool info = %x"
...
#define LOG_LE_RX_FLOW_CTRL 13100 //"LE rx flow control: ce_counter: %d, free buffer size: %d, flow_ctrl:%d"
